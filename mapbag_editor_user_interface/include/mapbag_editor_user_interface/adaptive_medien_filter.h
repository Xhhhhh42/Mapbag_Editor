...
GridMap<float> meanFilter_impl( const hector_math::GridMap<float>& input, int &filterSize, hector_math::GridMap<float>& zmin, 
                                hector_math::GridMap<float>& zmax ) {
    int width = input.cols();
    int height = input.rows();
    GridMap<float> imageGray = input;
    // zmin = GridMap<float>::Zero(height, width) ;
    // zmax = GridMap<float>::Zero(height, width) ;
    hector_math::GridMap<float> zmed = GridMap<float>::Zero( height, width );
    for ( int i = 0; i < height; i++ ) {
        for ( int j = 0; j < width; j++ ) {
            int x1 = std::max( i - filterSize / 2, 0 );
            int x2 = std::min( i + filterSize / 2, height - 1 );
            int y1 = std::max( j - filterSize / 2, 0 );
            int y2 = std::min( j + filterSize / 2, width - 1 );
...
GridMap<float> meanFilter( const hector_math::GridMap<float>& input, int &filterSize ) {
    int width = input.cols();
    int height = input.rows();
    GridMap<float> zmin = GridMap<float>::Zero( height, width );
    GridMap<float> zmax = GridMap<float>::Zero( height, width );
    GridMap<float> zmed = meanFilter_impl( input, filterSize, zmin, zmax );
    return zmed;
}


GridMap<float> adaptiveMeanFilter( const hector_math::GridMap<float>& input, int &filterSize, int &maxSize ) {
    int width = input.cols();
    int height = input.rows();
    GridMap<float> imageGray = input;
    GridMap<float> output = GridMap<float>::Zero( height, width );
    BoolMap alreadyProcessed( height, width );
    alreadyProcessed.setZero();
    GridMap<float> zmed_copy = GridMap<float>::Zero( height, width );
    

    int Smax = maxSize;
    for ( int k = filterSize; k <= Smax; k += 2 ) {
        GridMap<float> zmin = GridMap<float>::Zero( height, width );
        GridMap<float> zmax = GridMap<float>::Zero( height, width );
        GridMap<float> zmed = meanFilter_impl( input, k, zmin, zmax );
        ...
        if ( alreadyProcessed.array().all() ) { break; }
    }
